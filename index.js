...
const { color, bgcolor } = require('./verienty/storage/color')
const fs = require("fs-extra")
const simple = require('./verienty/storage/simple.js')
const WAConnection = simple.WAConnection(_WAConnection)
const figlet = require('figlet')
const fetch = require('node-fetch')
const { getBuffer, info, start, success} = require('./verienty/storage/functions')
const moment = require("moment-timezone")
const gcdetect = JSON.parse(fs.readFileSync('./verienty/action/gcdetect.json'))
...
const welkom = JSON.parse(fs.readFileSync('./verienty/action/welkom.json'))
const {  location } = MessageType
const settings = JSON.parse(fs.readFileSync('./verienty/options/settings.json'))
baterai = 'unknown'
charging = 'unknown'
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	Fan.browserDescription = [`${settings.bailyes}`, `${randomserverku}`, '3.0']

	Fan.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color('SCAN QR NYA KONTOL'))
})
	Fan.on('credentials-updated', () => {
		fs.writeFileSync('./verienty/connection/kontol.json', JSON.stringify(Fan.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'IRFAN  LOADING...')
	})
	fs.existsSync('./verienty/connection/kontol.json') && Fan.loadAuthInfo('./verienty/connection/kontol.json')
	Fan.on('connecting', () => {
		start('2', 'CONNECT')
	})
	Fan.on('open', () => {
